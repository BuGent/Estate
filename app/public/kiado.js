...
async function conn(){
    ...
    let estateList = await fetch('http://localhost:3000/frisskiadoak', {
        method: 'GET',
        headers: {
...
function loadMore() {     
    ...
    document.getElementById('loadMore').onclick = async function(){
        ...
        let nextList = await fetch('http://localhost:3000/frisskiadoak', {
            method: 'POST',
            headers: {
                "Content-Type": 'application/json'
            },
        ...
        document.getElementById("kiadoEstates").innerHTML +=  renderEstates(nextList, hossz);
...
document.getElementById('kiadForm').onsubmit = async function (event){
    ...
    let szurtList = await fetch('http://localhost:3000/kiado', {
        method: 'POST',
        headers: {
            "Content-Type": 'application/json'
        },
    ...
    if (szurtList.length === 0){
        document.getElementById("kiadoEstates").innerHTML = "<p>Nincs megjeleníthető találat.</p>"
    }
    else{
        let hossz;
        ...
        document.getElementById("kiadoEstates").innerHTML =  renderEstates(szurtList, hossz);
        if (szurtList.length == 13){
            document.getElementById("more").innerHTML =  '<button type="button" id="loadMore">Több betöltése</button>';
...
function szurLoadMore(tipus, telepules, min, max) {     
    ...
    document.getElementById('loadMore').onclick = async function(){
        ...
        let nextSzurtList = await fetch('http://localhost:3000/kiado', {
            method: 'POST',
            headers: {
                "Content-Type": 'application/json'
            },
        ...
        document.getElementById("kiadoEstates").innerHTML +=  renderEstates(nextSzurtList, hossz);
        if (nextSzurtList.length == 13){
            document.getElementById("more").innerHTML =  '<button type="button" id="loadMore">Több betöltése</button>';
            szurLoadMore();
        }
...
function renderEstates(estateList, hossz){            
    ...
    for(let i=0; i < hossz; i++){   
        if(estateList[i].kepek > 0){
            kepek="http://localhost:3000/kepek/kiado/" + estateList[i].userid + "/" + estateList[i].hirdetesazon +"/100.jpg";
        }        
        else{
        ...
        estatesHTML += `           
            <div class="renderEstate" onclick="megjelenit(${estateList[i].hirdetesazon}, ${2}, ${estateList[i].userid})">
                ...
                <div class="renderEstateAdatok">     
                    ...
                    <div class="renderEstateAdat">    
                        <div class="renderEstateAr">${((estateList[i].ar)*1000).toLocaleString("de-DE")} Ft</div>
